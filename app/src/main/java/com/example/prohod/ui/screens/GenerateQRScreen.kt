...
import com.example.prohod.ui.theme.ButtonOutlinedBase
import com.example.prohod.ui.theme.TextStyleMedium
import com.example.prohod.ui.viewmodels.GenerateQRViewModel
...
fun GenerateQRScreen(navController: NavHostController) {
    ...
    val generateQRViewModel = hiltViewModel<GenerateQRViewModel>()
    ...
    ConstraintLayout(Modifier.fillMaxSize()) {
        ...
        generateQRViewModel.isRequestAccepted.observeAsState().value?.let {
            if (it.data == true) {
                isQRGenerated = true
            }
            if (it.data == false) {
                Toast.makeText(LocalContext.current, "Заявка на рассмотрении", Toast.LENGTH_SHORT)
                    .show()
            }
        }
        if (!isQRGenerated) {
            ButtonOutlinedBase(
                onClick = {
                    generateQRViewModel.updateIsRequestAccepted()
                },
                modifier = Modifier
            ...
            val qr =
                encodeAsBitmap("${generateQRViewModel.fullName}\n" +
                        "Паспорт 6522 686868\n" +
                        "Выдан 23.01.12\n" +
                        "\n" +
                        "Посещение одобрено")!!
            ...
            var t by remember { mutableIntStateOf(1200) }
